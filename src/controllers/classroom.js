...
async function newClassroom(req, res) {
    ...
    if (!(/^[A-Za-zÁÉÍÓÚáéíóú][A-Za-zÁÉÍÓÚáéíóú0-9 -\x41\x42]{2,50}$/.test(req.body.classroomName))) return res.status(400).send({ message: `the class room name must be between 2 and 50 characters, not contain spaces and empy start with a letter` });
